...
# These are sample questions that users can click on in the sidebar to quickly send a query to the chatbot. They cover common telecom issues related to connectivity, billing, SIM cards, roaming, and device unlocking.
st.set_page_config(
...
@st.cache_resource
# Building the RAG chain involves loading the retriever, embeddings, vector database, and LLM, which is an expensive operation. Using @st.cache_resource, Streamlit creates the chain only once and reuses it for every user query, making the application much faster.
def get_chain():
    return build_chain()

if "messages" not in st.session_state:
# I use session_state to store the chat history so that previous messages are not lost.
    st.session_state.messages = []
if "pending_question" not in st.session_state:
    st.session_state.pending_question = None
# pending question stores the sample question selected from the sidebar.
# ── Sidebar ──────────────────────────────────────────────────────────────────
with st.sidebar:
...
# This creates one button for every question.
